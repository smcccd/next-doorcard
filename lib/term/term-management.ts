...
// Term display values (used in forms)
export const TERM_DISPLAY_VALUES = ["Fall", "Spring", "Summer"] as const;
export type TermDisplayValue = (typeof TERM_DISPLAY_VALUES)[number];

// Campus/College values (used in forms)
export const CAMPUS_VALUES = ["SKYLINE", "CSM", "CANADA"] as const;
export type CampusValue = (typeof CAMPUS_VALUES)[number];

/**
 * Converts a display term season to the enum value.
 * @param display - The display value (e.g., "Fall", "Spring", "Summer")
 * @returns The enum value (e.g., "FALL", "SPRING", "SUMMER")
 */
export function toEnumSeason(display: TermDisplayValue): "FALL" | "SPRING" | "SUMMER" {
  return display.toUpperCase() as "FALL" | "SPRING" | "SUMMER";
}
