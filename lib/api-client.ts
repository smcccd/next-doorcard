  ...
  private async makeRequest<T>(
    ...
    try {
      const response = this.enableDeduplication 
        ? await requestDeduplicator.deduplicate(url, makeRequest, requestOptions)
        : await makeRequest();
...
export const api = {
  ...
  doorcards: {
    ...
    update: (id: string, data: any) => apiClient.patch(`/api/doorcards/${id}`, data),
  ...
  admin: {
    ...
    getAnalytics: () => apiClient.get('/api/admin/analytics', fetchPresets.heavy),
