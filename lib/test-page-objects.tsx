...
export class HomePageObject {
  ...
  async expectPageHeader() {
    ...
    expect(screen.getByText(/office hours.*contact information/i)).toBeInTheDocument();
  }
  ...
  async selectDepartment(department: string) {
    ...
    const option = screen.getByRole('option', { name: new RegExp(department, 'i') });
  ...
  getProfessorCards() {
    ...
    const cards = screen.queryAllByRole('article') ||
                  ...
                  screen.queryAllByText(/office.*hours/i).map(el => el.closest('[class*="card"]')).filter(Boolean);
  ...
  expectProfessorCardContains(text: string, index: number = 0) {
    const cards = this.getProfessorCards();
    expect(within(cards[index]).getByText(new RegExp(text, 'i'))).toBeInTheDocument();
  }
