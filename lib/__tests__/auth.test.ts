...
const mockPrismaAdapter = PrismaAdapter as jest.MockedFunction<typeof PrismaAdapter>;
const mockCredentialsProvider = CredentialsProvider as jest.MockedFunction<typeof CredentialsProvider>;
...
describe('Auth Configuration', () => {
  ...
  describe('OneLogin Provider', () => {
    it('should configure OneLogin provider correctly', () => {
      const oneLoginProvider = authOptions.providers.find((p: any) => p.id === 'onelogin');
    ...
    it('should have correct authorization configuration', () => {
      ...
      expect(oneLoginProvider?.authorization?.url).toBe('https://smccd.onelogin.com/oidc/2/auth');
      expect(oneLoginProvider?.authorization?.params?.scope).toBe('openid profile email');
      expect(oneLoginProvider?.authorization?.params?.response_type).toBe('code');
    });

    it('should have correct token endpoint configuration', () => {
      const oneLoginProvider = authOptions.providers.find((p: any) => p.id === 'onelogin');
      
      expect(oneLoginProvider?.token?.url).toBe('https://smccd.onelogin.com/oidc/2/token');
    ...
    it('should have correct userinfo endpoint configuration', () => {
      ...
      expect(oneLoginProvider?.userinfo?.url).toBe('https://smccd.onelogin.com/oidc/2/me');
  ...
  describe('Credentials Provider', () => {
    it('should be configured for development only', () => {
      const credentialsProvider = authOptions.providers.find((p: any) => p.type === 'credentials');
