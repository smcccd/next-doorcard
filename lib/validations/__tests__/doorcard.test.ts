...
describe('Doorcard Validation Schemas', () => {
  ...
  describe('appointmentCategorySchema', () => {
    ...
    it('rejects invalid appointment categories', () => {
      expect(() => appointmentCategorySchema.parse('INVALID_CATEGORY')).toThrow()
  ...
  describe('appointmentSchema', () => {
    ...
    it('validates that end time is after start time', () => {
      ...
      expect(() => appointmentSchema.parse(invalidAppointment)).toThrow('End time must be after start time')
    })
    ...
    it('validates required fields', () => {
      expect(() => appointmentSchema.parse({ ...validAppointment, name: '' })).toThrow('Appointment name is required')
      expect(() => appointmentSchema.parse({ ...validAppointment, startTime: '' })).toThrow()
      expect(() => appointmentSchema.parse({ ...validAppointment, endTime: '' })).toThrow()
      expect(() => appointmentSchema.parse({ ...validAppointment, dayOfWeek: '' })).toThrow()
    })
  ...
  describe('basicInfoSchema', () => {
    ...
    it('validates required fields', () => {
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, name: '' })).toThrow('Name is required')
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, doorcardName: '' })).toThrow('Doorcard name is required')
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, officeNumber: '' })).toThrow('Office number is required')
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, term: '' })).toThrow('Invalid enum value')
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, year: 1999 })).toThrow('Number must be greater than or equal to 2020')
    })
    ...
    it('validates year format', () => {
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, year: 2019 })).toThrow('Number must be greater than or equal to 2020')
      expect(() => basicInfoSchema.parse({ ...validBasicInfo, year: 2031 })).toThrow('Number must be less than or equal to 2030')
    })
    ...
    it('rejects invalid date range', () => {
      ...
      expect(() => basicInfoSchema.parse(invalidWithDates)).toThrow('End date must be after start date')
    })
  })
  ...
  describe('doorcardSchema', () => {
    ...
    it('validates nested appointment data', () => {
      ...
      expect(() => doorcardSchema.parse(invalidDoorcard)).toThrow('Appointment name is required')
    })
  })
  ...
  describe('updateDoorcardSchema', () => {
    ...
    it('validates provided fields', () => {
      expect(() => updateDoorcardSchema.parse({ name: '' })).toThrow('Name is required')
      expect(() => updateDoorcardSchema.parse({ year: 1999 })).toThrow('Number must be greater than or equal to 2020')
    })
  ...
  describe('userSchema', () => {
    ...
    it('validates email format', () => {
      expect(() => userSchema.parse({ email: 'invalid-email' })).toThrow('Invalid email address')
      expect(() => userSchema.parse({ email: 'test@' })).toThrow('Invalid email address')
      expect(() => userSchema.parse({ email: '@domain.com' })).toThrow('Invalid email address')
    })
  ...
  describe('timeBlockSchema', () => {
    ...
    it('validates required fields', () => {
      expect(() => timeBlockSchema.parse({ ...validTimeBlock, activity: '' })).toThrow('Activity is required')
    })
...
describe('Validation Helper Functions', () => {
  ...
  describe('validateTimeBlockOverlap', () => {
    const existingBlocks = [
      { id: '1', day: 'MONDAY', startTime: '09:00', endTime: '10:00', activity: 'Office Hours' },
      { id: '2', day: 'MONDAY', startTime: '11:00', endTime: '12:00', activity: 'Class' },
      { id: '3', day: 'TUESDAY', startTime: '09:00', endTime: '10:00', activity: 'Meeting' },
    ]
    ...
    it('returns error message when overlap exists', () => {
      ...
      expect(result).toContain('Time block overlaps with existing MONDAY block')
    ...
    it('handles blocks on different days', () => {
      const newBlock = { day: 'WEDNESDAY', startTime: '09:00', endTime: '10:00' }
      expect(validateTimeBlockOverlap(newBlock, existingBlocks)).toBeNull()
    })
...
describe('Edge Cases and Error Handling', () => {
  ...
  describe('Complex validation scenarios', () => {
    it('validates doorcard with many appointments', () => {
      const manyAppointments = Array(20).fill(null).map((_, i) => {
