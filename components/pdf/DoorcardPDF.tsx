      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.officeInfo}>
          <View style={styles.officeItem}>
            ...
            <Text style={styles.officeValue}>{doorcard.officeNumber || 'TBA'}</Text>
          </View>
          <View style={styles.officeItem}>
                  ...
                  <View key={slot.time} style={styles.timeSlot}>
                    ...
                    {appointment ? (
                      <View style={[
                        styles.appointment,
                        { 
                          borderLeftColor: CATEGORY_COLORS[appointment.category as keyof typeof CATEGORY_COLORS] || CATEGORY_COLORS.OTHER,
                          backgroundColor: appointment.category === 'OFFICE_HOURS' ? '#dbeafe' :
                                         appointment.category === 'CLASS' ? '#d1fae5' :
                                         appointment.category === 'MEETING' ? '#e7d3ff' :
                                         appointment.category === 'RESEARCH' ? '#fef3c7' : '#f3f4f6'
                        }
                      ]}>
                        <Text style={[
                          styles.appointmentText,
                          { 
                            color: appointment.category === 'OFFICE_HOURS' ? '#1e40af' :
                                   appointment.category === 'CLASS' ? '#047857' :
                                   appointment.category === 'MEETING' ? '#7c3aed' :
                                   appointment.category === 'RESEARCH' ? '#d97706' : '#4b5563'
                          }
                        ]}>
                          {appointment.name}
                        </Text>
            ...
            <View style={styles.legendItems}>
              {categories.map(category => (
                <View key={category} style={styles.legendItem}>
                  <View style={[
                    styles.legendColor,
                    { backgroundColor: CATEGORY_COLORS[category as keyof typeof CATEGORY_COLORS] || CATEGORY_COLORS.OTHER }
                  ]} />
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            Generated from Faculty Doorcard System • {new Date().toLocaleDateString()}
          </Text>
          <Text style={styles.footerText}>
...
export function DoorcardPDF({ doorcard, doorcardId, onDownload }: DoorcardPDFWrapperProps) {
