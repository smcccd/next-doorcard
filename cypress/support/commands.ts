...
Cypress.Commands.add("checkServer", () => {
  cy.task('ping').then((isServerRunning) => {
    if (!isServerRunning) {
      throw new Error(
        'Server is not running at ' + Cypress.config('baseUrl') + 
...
Cypress.Commands.add("login", (email, password) => {
  cy.session(
    ...
    () => {
      ...
      cy.contains('Show development login', { timeout: 10000 }).should('be.visible').click();
...
Cypress.Commands.add("loginAsTestUser", () => {
  cy.session("test-user-session", () => {
    ...
    // Wait for the form to appear and then fill it out
    cy.get('input[name="email"]', { timeout: 10000 }).should('be.visible').clear().type("besnyib@smccd.edu");
    cy.get('input[name="password"]', { timeout: 10000 }).should('be.visible').clear().type("password123");
    cy.get('button[type="submit"]').should('be.visible').click();
...
Cypress.Commands.add("fastLogin", (userEmail = "besnyib@smccd.edu", userName = "Test User") => {
  cy.session(
...
Cypress.Commands.add("simpleLogin", (userEmail = "besnyib@smccd.edu") => {
  cy.visit("/login");
  cy.contains('Show development login', { timeout: 10000 }).should('be.visible').click();
  cy.get('input[name="email"]', { timeout: 10000 }).should('be.visible').clear().type(userEmail);
  cy.get('input[name="password"]', { timeout: 10000 }).should('be.visible').clear().type("password123");
  cy.get('button[type="submit"]').should('be.visible').click();
  cy.url({ timeout: 15000 }).should("include", "/dashboard");
  ...
  cy.request({
    ...
    if (response.status === 200 && response.body.doorcards) {
      response.body.doorcards.forEach((doorcard: any) => {
        if (doorcard.name?.includes("Test") || 
...
Cypress.Commands.add("submitFormAndCheckValidation", (expectedErrors = []) => {
  cy.get('button[type="submit"], button:contains("Next"), button:contains("Submit")').click();
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      waitForElement(selector: string, options?: any): Chainable<JQuery<HTMLElement>>;
      ...
      submitFormAndCheckValidation(expectedErrors?: string[]): Chainable<boolean>;
    }
  }
}
