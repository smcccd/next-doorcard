...
async function checkAvailableData() {
  ...
  try {
    ...
    const availableTerms = await prisma.$queryRaw<Array<{
      term: string;
      year: number;
    ...
    availableTerms.forEach(item => {
      console.log(`${item.term} ${item.year}: ${item.doorcard_count} total doorcards (${item.active_count} active)`);
    });

    // Check campuses
    console.log('\n🏫 Available Campuses:');
    const availableCampuses = await prisma.$queryRaw<Array<{
      college: string | null;
      doorcard_count: bigint;
      user_count: bigint;
    }>>`
    ...
    availableCampuses.forEach(item => {
      ...
      console.log(`${campus}: ${item.doorcard_count} doorcards, ${item.user_count} users`);
    });
    ...
    const recentDoorcards = await prisma.$queryRaw<Array<{
      term: string;
      year: number;
      college: string | null;
      created_at: Date;
    ...
    recentDoorcards.forEach((item, index) => {
      ...
      console.log(`${(index + 1).toString().padStart(2)}. ${item.term} ${item.year} | ${campus} | ${status} | ${item.created_at.toISOString().split('T')[0]}`);
    });
