...
async function analyzeRejects() {
  ...
  Object.entries(doorcardRejectReasons)
    ...
    .forEach(([reason, count]) => {
      console.log(`  ${reason}: ${count} (${(count/doorcardRejects.length*100).toFixed(1)}%)`);
    });
  ...
  await new Promise((resolve, reject) => {
    fs.createReadStream('/Users/besnyib/next-doorcard/rejects/TBL_APPOINTMENT.csv')
      .pipe(parse({ headers: true }))
  ...
  appointmentRejects.forEach(row => {
    const reason = row._reject_reason;
    appointmentRejectReasons[reason] = (appointmentRejectReasons[reason] || 0) + 1;
  });
  ...
  Object.entries(appointmentRejectReasons)
    .sort((a, b) => b[1] - a[1])
    .forEach(([reason, count]) => {
      console.log(`  ${reason}: ${count} (${(count/appointmentRejects.length*100).toFixed(1)}%)`);
    });
  ...
  doorcardRejects.forEach(row => {
    if (row.college) {
      collegeDistribution[row.college] = (collegeDistribution[row.college] || 0) + 1;
    }
  });
  ...
  const rejectionRate = ((doorcardRejects.length + appointmentRejects.length) / (10945 + 184936)) * 100;
  ...
  console.log(`Appointment rejection rate: ${appointmentRejectionRate.toFixed(1)}%`);
  
  console.log('\n⚠️  Issues requiring immediate attention:');
  console.log(`1. ${appointmentRejects.length} appointments rejected (${appointmentRejectionRate.toFixed(1)}% of total)`);
  console.log(`2. ${missingUsers.size} unique usernames not found in user table`);
  ...
  const emptyUsernameAppointments = appointmentRejects.filter(row => 
    !row.username || row.username.trim() === ''
  );
  console.log(`5. ${emptyUsernameAppointments.length} appointments with completely empty usernames`);
  ...
  console.log(`Successful doorcards: ${successfulDoorcards} (${((successfulDoorcards/10945)*100).toFixed(1)}%)`);
  console.log(`Successful appointments: ${successfulAppointments} (${((successfulAppointments/184936)*100).toFixed(1)}%)`);
  
  // Recommendations
  console.log('\n💡 RECOMMENDATIONS FOR PRODUCTION:');
  console.log('1. Fix user creation process - create users from ALL doorcard/appointment data first');
  console.log('2. Handle empty usernames in appointments by linking to doorcard owners');
  console.log('3. Implement user matching by alternative identifiers (email patterns, name matching)');
  ...
  console.log('6. Implement better error handling for malformed dates and terms');
}
