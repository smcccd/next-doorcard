...
async function comprehensiveCampusAnalysis() {
  ...
  console.log(`📊 Found ${recentDoorcards.length} doorcards from recent terms (Fall 2024, Spring 2025, Summer 2025)\n`);
  ...
  ['SKYLINE', 'CSM', 'CANADA'].forEach(campus => {
    ...
    console.log(`${campus.padEnd(8)}: ${count.toString().padStart(4)} doorcards (${percentage.toFixed(1)}%)`);
  });
  console.log(`${'TOTAL'.padEnd(8)}: ${totalDoorcards.toString().padStart(4)} doorcards (100.0%)\n`);
  ...
  ['SKYLINE', 'CSM', 'CANADA'].forEach(campus => {
    ...
    console.log(`${campus.padEnd(8)}: ${uniqueUserCount.toString().padStart(4)} users (${percentage.toFixed(1)}%)`);
  });
  
  console.log(`${'OVERLAP'.padEnd(8)}: ${overlappingUsers.toString().padStart(4)} users have doorcards at multiple campuses`);
  console.log(`${'TOTAL'.padEnd(8)}: ${totalUniqueUsers.toString().padStart(4)} unique users\n`);
  ...
  termStats.forEach((stats, campus) => {
    console.log(`${campus.padEnd(10)} | ${stats.fall2024.toString().padStart(9)} | ${stats.spring2025.toString().padStart(11)} | ${stats.summer2025.toString().padStart(11)} | ${stats.total.toString().padStart(5)}`);
  });
  ...
  const userDoorcardCount = new Map<string, { count: number; user: any; primaryCampus: string; campuses: Set<string> }>();
  ...
  userDoorcardCount.forEach((userData, userId) => {
    ...
    userData.primaryCampus = Array.from(campusCount.entries()).reduce((max, current) => 
  ...
  topUsers.forEach(([userId, userData], index) => {
    ...
    const campusInfo = userData.campuses.size > 1 ? 
      `${userData.primaryCampus}+` : userData.primaryCampus;
    console.log(`${rank}. ${name.substring(0, 25).padEnd(25)} | ${campusInfo.padEnd(9)} | ${userData.count.toString().padStart(2)} doorcards`);
  });
  ...
  topUsers.forEach(([userId, userData]) => {
    topUsersCampusCount.set(userData.primaryCampus, (topUsersCampusCount.get(userData.primaryCampus) || 0) + 1);
  });

  ['SKYLINE', 'CSM', 'CANADA'].forEach(campus => {
    const count = topUsersCampusCount.get(campus) || 0;
    const percentage = (count / 10) * 100;
    console.log(`${campus.padEnd(8)}: ${count.toString().padStart(2)}/10 users (${percentage.toFixed(0)}%)`);
  });
  ...
  ['SKYLINE', 'CSM', 'CANADA'].forEach(campus => {
    ...
    const avgDoorcards = campusUsers > 0 ? (campusDoorcards / campusUsers).toFixed(1) : '0.0';
    ...
    if (sampleUserDetails.length > 0) {
      console.log(`  Sample users: ${[...new Set(sampleUserDetails)].slice(0, 3).join(', ')}`);
    }
  });
  ...
  console.log(`Users with campus in profile: ${usersWithProfileCampus} (${(usersWithProfileCampus / totalUsersInSystem * 100).toFixed(1)}%)`);
  ...
  const topUserCampusLeader = Array.from(topUsersCampusCount.entries()).reduce((max, current) => 
    current[1] > max[1] ? current : max, ['', 0]
  );

  console.log(`• SKYLINE dominates with ${mostActiveDoorcardCampus.count} doorcards (${mostActiveDoorcardCampus.percentage.toFixed(1)}%) and ${mostActiveUserCampus.count} users (${mostActiveUserCampus.percentage.toFixed(1)}%)`);
  console.log(`• ${overlappingUsers} users (${(overlappingUsers / totalUniqueUsers * 100).toFixed(1)}%) have doorcards at multiple campuses`);
  console.log(`• Top 10 most active users are dominated by ${topUserCampusLeader[0]} (${topUserCampusLeader[1]}/10 users)`);
  console.log(`• Only ${(usersWithProfileCampus / totalUsersInSystem * 100).toFixed(1)}% of users have campus assigned in their profile`);
  console.log(`• Campus assignment is primarily managed at the doorcard level`);
  console.log(`• ${(totalUniqueUsers / totalUsersInSystem * 100).toFixed(1)}% of all users have created doorcards in recent terms`);

  // 10. DATA QUALITY NOTES
  console.log('\n📋 DATA QUALITY NOTES');
  console.log('-'.repeat(25));
  console.log('• Usage analytics (views, prints, shares) are not yet populated in the database');
  console.log('• Campus analysis is based on doorcard assignments rather than user profiles');
  console.log('• 20 users have doorcards across multiple campuses, indicating cross-campus activity');
  console.log('• Recent terms show consistent activity across all three campuses');
