...
async function finalProductionAnalysis() {
  ...
  try {
    ...
    doorcardsByStatus.forEach(group => {
      console.log(`Doorcards ${group.isActive ? 'active' : 'inactive'}: ${group._count.isActive}`);
    });
    ...
    console.log('\n📅 TEMPORAL DATA RANGE:');
    console.log(`Oldest doorcard: ${oldestDoorcard.term} ${oldestDoorcard.year}`);
    console.log(`Newest doorcard: ${newestDoorcard.term} ${newestDoorcard.year}`);
  ...
  console.log('1. APPOINTMENT IMPORT FAILURE: Complete failure despite appearing successful');
  console.log('2. USER DATA INADEQUACY: Source contains only 2 users vs 2000+ needed');
  console.log('3. ORPHANED DATA: 10,000+ placeholder doorcards with no real appointments');
  console.log('4. INTEGRITY VIOLATIONS: Foreign key relationships are broken');
  console.log('5. BUSINESS LOGIC ISSUES: No active doorcards, no real user associations');
  
  console.log('\n📊 STATISTICAL SUMMARY:');
  console.log('• Source Files: 185K appointments, 11K doorcards, 2 users');
  console.log('• Import Success: 10K doorcards, 0 appointments, 1792 auto-generated users');
