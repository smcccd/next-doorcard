...
async function analyzeCampusDistribution() {
  console.log('🏫 Analyzing Campus Distribution for Doorcard Users (2024-2025)');
  ...
  recentDoorcards.forEach(doorcard => {
    ...
    campusDistribution.set(doorcardCampus, (campusDistribution.get(doorcardCampus) || 0) + 1);
  ...
  recentDoorcards.forEach(doorcard => {
    ...
    if (!userActivityMap.has(userId)) {
      ...
      const primaryCampus = Array.from(campusCount.entries()).reduce((max, current) => 
  ...
  userActivityMap.forEach(userData => {
    userData.totalActivity = userData.totalViews + (userData.totalPrints * 3) + (userData.totalShares * 2);
  });
  ...
  const campusActivityStats = new Map<string, {
    totalViews: number;
    totalPrints: number;
    totalShares: number;
    userCount: number;
  ...
  campusActivityStats.forEach((stats, campus) => {
    console.log(`\n${campus}:`);
    console.log(`  Users: ${stats.userCount} | Doorcards: ${stats.doorcardCount}`);
    console.log(`  Total Activity: ${stats.totalViews} views, ${stats.totalPrints} prints, ${stats.totalShares} shares`);
    console.log(`  Avg per User: ${stats.avgViewsPerUser.toFixed(1)} views, ${stats.avgPrintsPerUser.toFixed(1)} prints, ${stats.avgSharesPerUser.toFixed(1)} shares`);
  });
  ...
  console.log(`• Most doorcards: ${mostActiveDoorcardCampus.campus} (${mostActiveDoorcardCampus.count} doorcards, ${mostActiveDoorcardCampus.percentage.toFixed(1)}%)`);
  console.log(`• Most users: ${mostActiveUserCampus.campus} (${mostActiveUserCampus.count} users, ${mostActiveUserCampus.percentage.toFixed(1)}%)`);
  ...
  if (topUserCampusLeader[0]) {
    console.log(`• Top users dominated by: ${topUserCampusLeader[0]} (${topUserCampusLeader[1]}/10 users)`);
  }
  ...
  console.log(`• User profile campus assignment: ${totalUsers - usersWithNoCampusProfile}/${totalUsers} users have campus in profile (${((totalUsers - usersWithNoCampusProfile) / totalUsers * 100).toFixed(1)}%)`);
  console.log(`• Note: Campus analysis based on doorcard assignments since most users lack profile campus data`);
