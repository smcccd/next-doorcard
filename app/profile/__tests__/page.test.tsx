...
describe('ProfilePage', () => {
  ...
  describe('Profile Display', () => {
    ...
    it('should show selected title', async () => {
      ...
      await waitFor(() => {
        ...
        const titleSelect = selects.find(s => s.querySelector('option[value="Dr."]'));
    ...
    it('should show selected pronouns', async () => {
      ...
      await waitFor(() => {
        const selects = screen.getAllByTestId('select');
        const pronounsSelect = selects.find(s => s.querySelector('option[value="they/them"]'));
        expect(pronounsSelect?.querySelector('select')).toHaveValue('they/them');
      });
    });
    ...
    it('should show selected campus', async () => {
      ...
      await waitFor(() => {
        const selects = screen.getAllByTestId('select');
        const campusSelect = selects.find(s => s.querySelector('option[value="SKYLINE"]'));
  ...
  describe('Display Name Preview', () => {
    it('should update preview when changing display format', async () => {
      ...
      const selects = screen.getAllByTestId('select');
      const displayFormatSelect = selects.find(s => 
        s.querySelector('option[value="FIRST_LAST"]')
      )?.querySelector('select');

      if (displayFormatSelect) {
        fireEvent.change(displayFormatSelect, { target: { value: 'FIRST_LAST' } });
  ...
  describe('Local Storage', () => {
    it('should clear profile setup dismissal on mount', async () => {
      ...
      await waitFor(() => {
        expect(localStorage.getItem('profile-setup-dismissed-user-123')).toBeNull();
      });
    });
  });
