...
describe('Doorcard Server Actions', () => {
  ...
  describe('createDoorcardWithCampusTerm', () => {
    ...
    it('should return error for duplicate doorcard', async () => {
      ...
      const result = await createDoorcardWithCampusTerm({ success: true }, formData);
  ...
  describe('validateCampusTerm', () => {
    ...
    it('should return error for non-existent doorcard', async () => {
      ...
      const result = await validateCampusTerm('non-existent', { success: true }, formData);
    ...
    it('should return error for duplicate term combination', async () => {
      mockPrisma.doorcard.findUnique.mockResolvedValue(mockDoorcard as any);
      mockPrisma.doorcard.findFirst.mockResolvedValue({ id: 'other-doorcard' } as any);
      ...
      const result = await validateCampusTerm('doorcard-123', { success: true }, formData);
  ...
  describe('updateBasicInfo', () => {
    ...
    it('should return error for invalid data', async () => {
      ...
      const result = await updateBasicInfo('doorcard-123', { success: true }, formData);
    ...
    it('should handle non-existent doorcard', async () => {
      ...
      const result = await updateBasicInfo('non-existent', { success: true }, formData);
  ...
  describe('publishDoorcard', () => {
    ...
    it('should handle non-existent doorcard', async () => {
      ...
      await expect(publishDoorcard('non-existent')).rejects.toThrow('Doorcard not found');
    });
    ...
    it('should handle authentication errors', async () => {
      ...
      await expect(publishDoorcard('doorcard-123')).rejects.toThrow('Unauthorized');
    });
  });
  ...
  describe('deleteDoorcard', () => {
    ...
    it('should handle database errors gracefully', async () => {
      mockPrisma.doorcard.findUnique.mockResolvedValue(mockDoorcard as any);
      mockPrisma.appointment.deleteMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('Campus Label Helper', () => {
    ...
    it('should provide correct campus labels in error messages', async () => {
      mockPrisma.doorcard.findFirst.mockResolvedValue({ id: 'existing' } as any);
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors', async () => {
      mockPrisma.doorcard.findFirst.mockRejectedValue(new Error('Database connection failed'));
