...
describe('/api/doorcards', () => {
  ...
  describe('POST /api/doorcards', () => {
    ...
    it('returns 401 when user is not authenticated', async () => {
      mockRequireAuthUserAPI.mockResolvedValue({ error: 'Unauthorized', status: 401 })
    ...
    it('returns 409 when doorcard already exists for same campus/term/year', async () => {
      ...
      expect(data.error).toContain('You already have a doorcard for Skyline College - FALL 2024')
    ...
    it('generates correct campus name in error message', async () => {
      ...
      for (const { college, expectedName } of testCases) {
        const existingDoorcard = { id: 'existing', userId: mockUser.id, college, term: 'FALL', year: 2024, isActive: true }
    ...
    it('generates URL-friendly slug correctly', async () => {
      ...
      for (const { name, term, year, expectedSlug } of testCases) {
        ...
        const expectedResult = { ...mockCreatedDoorcard, name, term, year, slug: expectedSlug }
    ...
    it('handles database errors', async () => {
      mockPrisma.doorcard.create.mockRejectedValue(new Error('Database connection failed'))
  ...
  describe('GET /api/doorcards', () => {
    ...
    it('handles database errors', async () => {
      mockPrisma.doorcard.findMany.mockRejectedValue(new Error('Database connection failed'))
  ...
  describe('Archived doorcard protection', () => {
    ...
    it('should prevent editing archived doorcards', async () => {
      ...
      const routeCode = fs.readFileSync(path.join(__dirname, '../[id]/route.ts'), 'utf8')
