...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { name, firstName, lastName, title, pronouns, displayFormat, college, website } = await req.json();
    console.log("[Profile API] Update request:", { name, firstName, lastName, title, pronouns, displayFormat, college, website });

    // Validate required fields - either use new firstName/lastName or legacy name
    const hasNewNameFields = firstName && lastName;
    const hasLegacyName = name && typeof name === "string" && name.trim().length > 0;
    ...
    if (hasNewNameFields) {
      ...
      updateData.title = title && title.trim() && title !== "none" ? title.trim() : null;
      updateData.pronouns = pronouns && pronouns.trim() && pronouns !== "none" ? pronouns.trim() : null;
    ...
    if (college && college !== "none" && ["SKYLINE", "CSM", "CANADA"].includes(college)) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log("[Profile API GET] Request headers:", Object.fromEntries(req.headers.entries()));
    
    const session = await getServerSession(authOptions);
    console.log("[Profile API GET] Session:", session ? "EXISTS" : "NULL");
    console.log("[Profile API GET] Full session:", JSON.stringify(session, null, 2));
